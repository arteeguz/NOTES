...
function Get-UserChoice {
    ...
    $form.Size = New-Object System.Drawing.Size(800,600)  # Increased the size
    ...
    $label.Location = New-Object System.Drawing.Point(20,40)  # Adjusted for larger form
...
function Process-SingleIP {
    ...
    $form.Size = New-Object System.Drawing.Size(800,400)  # Increased the size
    $form.StartPosition = 'CenterScreen'
    $form.FormBorderStyle = [System.Windows.Forms.FormBorderStyle]::FixedDialog
    $form.MaximizeBox = $false
    $form.MinimizeBox = $false
...
function Process-Segment {
    ...
    if ($form.Tag -eq 'back') {
        ...
        $segment = $form.Tag
        $ips = 0..255 | ForEach-Object { "$segment.$_" }  # Generate IPs from 0 to 255 within the segment
...
function Save-OutputData {
    if ($global:skipSave) { return }  # Skip saving if the 'back' button was pressed
    $outputPath = "$([Environment]::GetFolderPath('Desktop'))\output.csv"
...
$global:skipSave = $false  # Global variable to control saving

while ($true) {
    $choice = Get-UserChoice  # Get user's choice from the form
    if ($choice -eq $null) {
        break  # Exit the loop if user chose to close
    }
    ...
    switch ($choice) {
        '1' {
            ...
            if ($device -eq $null) {
                $global:skipSave = $true  # Indicate that user chose to go back
                continue  # Restart the loop if user chose to go back
            }
            $devices += $device  # Process single IP
        }
        '2' {
            ...
            if ($csvPath -and (Test-Path -Path $csvPath)) {
                ...
                [System.Windows.Forms.MessageBox]::Show("CSV file not found or not selected.", "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
                $global:skipSave = $true  # Indicate that user chose to go back
                continue  # Restart the loop if CSV is not found or not selected
            }
        }
        '3' {
            ...
            $devices += $device  # Process IP segment
        }
        '4' {
            ...
            $devices += $device  # Process IP segments from CSV file
        }
        default {
    ...
    # Reset skip save flag if processing continues
