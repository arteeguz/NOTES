...
function Validate-IPSegment {
    ...
    return $segment -match '^\d{1,3}\.\d{1,3}\.\d{1,3}$' -and $segment -split '\.' | ForEach-Object { $_ -ge 0 -and $_ -le 255 }
}
...
function Process-SingleIP {
    ...
    if (-not (Validate-IP -ip $ip)) {
        [System.Windows.Forms.MessageBox]::Show("Invalid IP address.", "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
        return $null
    }
...
function Process-Segment {
    ...
    if (-not (Validate-IPSegment -segment $segment)) {
        [System.Windows.Forms.MessageBox]::Show("Invalid IP segment.", "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
        return $null
    }
    ...
    if ($csvPath -and (Test-Path -Path $csvPath)) {
        ...
        foreach ($segment in $segments) {
            if (Validate-IPSegment -segment $segment.Segment) {
                $ips = 1..254 | ForEach-Object { "$($segment.Segment).$_" }
...
switch ($choice) {
    ...
    '4' {
        $devices += Process-SegmentFile  # Process IP segments from file
    }
    default {
